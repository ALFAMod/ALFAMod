...
if [ ! -n "$BASH" ]; then
	...
	bash -c '. <('"$(command -v curl >/dev/null && echo 'curl -Ls' || echo 'wget -qO-')"' https://github.com/AmoreForever/🕸️ALFAMod/raw/master/install.sh) '"$*"
	exit $?
fi
...
banner() {
	...
	clear
	printf "\n\e[1;35;47m                  \e[0m"
printf "\n\e[1;35;47m 🕸️ALFAMod BOT\e[0m"
printf "\n\e[1;35;47m 🕸️ALFAMod BOT\e[0m"
printf "\n\e[1;35;47m                  \e[0m"
printf "\n\n\e[3;34;40m 🕸️ALFAMod - Юзербот нового поколения \e[0m\n\n"
...
if [ -d "🕸️ALFAMod/friendly-telegram" ]; then
	cd 🕸️ALFAMod || {
...
${SUDO_CMD}rm -rf 🕸️ALFAMod
# shellcheck disable=SC2086
runout ${SUDO_CMD}git clone https://github.com/AmoreForever/🕸️ALFAMod|| {
