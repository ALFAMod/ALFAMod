...
class CoreMod(loader.Module):
    ...
    strings = {
        ...
        "what_prefix": "❓ <b>Какой должен быть префикс? </b>",
        "prefix_incorrect": "🚫 <b>Префикс должен состоять из одного символа. </b>",
        "prefix_set": "🪄 <b>Префикс изменен. Введите</b> <code>{newprefix}setprefix {oldprefix}</code> <b>чтобы вернуть префикс</b>",
    ...
    async def 🕸️ALFAModcmd(self, message: Message) -> None:
        """Get 🕸️ALFAMod version"""
    ...
    async def changescmd(self, message: Message) -> None:
        """Changes 🕸️ALFAMod"""

        await self.inline.form(
                    self.strings("changes", message),
                    reply_markup=[
