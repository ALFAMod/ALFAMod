...
class CloudBackend:
    ...
    async def _find_data_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if dialog.name == f"🕸️ ALFAMod-{self._me.user_id}-logs" and dialog.is_channel:
            ...
            return (
                await self._client(
                    CreateChannelRequest(
                        f"🕸️ ALFAMod-{self._me.user_id}-logs",
                        "🕸️ ALFAMod loglar guruhi bu guruhni o'chirmang aks holda muammolar yuzberadi!!!",
                        megagroup=True,
                    )
                )
            ).chats[0]
    ...
    async def _find_asset_channel(self):
        async for dialog in self._client.iter_dialogs(None, ignore_migrated=True):
            if (
                dialog.name == f"ALFAMod-{self._me.user_id}-assets"
    ...
    async def _make_asset_channel(self):
        async with self._anti_double_asset_lock:
            ...
            return (
                await self._client(
                    CreateChannelRequest(
                        f"🕸️ ALFAMod-{self._me.user_id}-assets",
                        "🕸️ ALFAMod loglar guruhi iltimos bu guruhni o'chirmang aks holda muammolar yuz beradi!!!",
                        megagroup=True,
                    )
                )
            ).chats[0]
